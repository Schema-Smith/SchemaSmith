...
my-northwind/
  ...
  .json-schemas/                   ← JSON Schema validation, generated on the fly
                                     (each file above references its own, so editors just work)
  Templates/
    Northwind/
      Template.json
      Tables/
...
No GUI editor needed -- the schema package is designed to read clearly in any editor or IDE. Open `my-northwind/` in VS Code, JetBrains Rider, Visual Studio, or whatever your team uses. The `.json-schemas/` folder holds a JSON Schema per file type, generated from the live engine types, and every package file opens with a `$schema` line pointing at its own -- so autocomplete, type checking and inline errors work as soon as you open a file, with nothing to configure. They are also what a CI job validates against; see [Testing and validation](06-testing-and-validation.md).
