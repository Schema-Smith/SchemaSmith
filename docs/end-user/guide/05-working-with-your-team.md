...
- **Lean on the `.json-schemas/` files.** SchemaTongs writes JSON Schema validation files on the fly. Map them in your IDE once (a `json.schemas` entry in VS Code, or the equivalent elsewhere) and you get autocomplete, type checking and inline errors while you're editing table JSON. Bad data types, misspelled property names, missing required fields -- caught at the editor before they hit a pull request. The same files are what a CI validation step reads, so the check is the same in both places.
